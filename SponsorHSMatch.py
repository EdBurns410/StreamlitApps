...
st.set_page_config(page_title="Company Matcher – MoneyNext", layout="wide")

# -----------------------------
# Configuration
# -----------------------------
PRIMARY_CONTACT_SCHEMA = [
    "Record ID",
    "First Name",
    "Last Name",
    "Create Date",
    ...
    "State Dropdown",
    "LinkedIn Url",
    "NA 25 Outreach Sequence",
    "Target Event Role",
    "BTS NA 25 Role",
    "Attendee_Cold_Status",
    "BTS NA 25 Attendance Status",
    "BTS NA 25 Date Registered",
    "Journey Stage",
    "Lifecycle Stage",
    "Expandi Team Member",
    "personalised registration link",
    "State/Region",
    "State/Region Code",
    "Country/Region",
    "Lifecycle Stage (EXPROM)",
...
# Corporate suffixes and fluff to strip for normalised matching
SUFFIXES = {
        ...
        for cc in company_cols:
            if cc in contacts.columns:
                ...
                if pd.notna(val) and str(val).strip():
                    variants.append((cc, str(val)))
        if not variants:
            continue
        for cc, raw in variants:
            long_rows.append({
                "__row_id__": ridx,
                "__company_source__": cc,
                "__company_raw__": raw,
                "__company_norm__": normalise_company(raw)
            })
    if not long_rows:
...
def run_matching(
    ...
    ci = company_index.merge(
        ...
        how="left",
        suffixes=("", "")
    ).drop_duplicates(subset=["__row_id__", "__company_norm__"])
    ...
    matched_rows = []
    contact_norm_series = ci["__company_norm__"]
    ...
    if matched_rows:
        ...
        grouped = matched_contacts_df.groupby("__target_company__", as_index=False).apply(
            lambda g: pd.Series({
                "unique_contacts": unique_contacts_count(g, id_col_candidates)
            })
    ...
    targets_full = targets_df.copy()
    for helper in ["__target_norm__"]:
        if helper in targets_full.columns:
            targets_full = targets_full.drop(columns=[helper])
...
def column_mapper_ui(df: pd.DataFrame, target_schema: List[str], title: str) -> Dict[str, str]:
    st.subheader(title)
    ...
    for r in range(rows):
        ...
        for c in range(ncols):
            ...
            sel = cols_row[c].selectbox(f"{target}", options, index=options.index(default))
...
# UI
# -----------------------------
st.title("Contact-to-Target Company Matcher")
...
with st.expander("Matching options", expanded=True):
    scorer_name = st.selectbox(
        ...
        help="token_set_ratio handles word order changes and duplicates"
    )
    score_cutoff = st.slider(
        "Minimum match score",
        min_value=60, max_value=100, value=88, step=1,
        help="Higher means stricter matches"
    )

tabs = st.tabs(["Primary workflow", "Alternate contacts schema"])

# -----------------------------
# Tab 1: Primary workflow
# -----------------------------
with tabs[0]:
    st.header("Primary workflow")
    c1, c2 = st.columns(2)
    with c1:
        contacts_file = st.file_uploader("Upload Contacts CSV (HubSpot export)", type=["csv"], key="contacts1")
    ...
    with c2:
        targets_file = st.file_uploader("Upload Target Companies CSV", type=["csv"], key="targets1")
    ...
    if not contacts_df_raw.empty:
        mapping_contacts = column_mapper_ui(contacts_df_raw, PRIMARY_CONTACT_SCHEMA, "Map Contacts columns to standard names")
        contacts_df = remap_df(contacts_df_raw, mapping_contacts)
    else:
        contacts_df = pd.DataFrame()

    if not targets_df_raw.empty:
        mapping_targets = column_mapper_ui(targets_df_raw, TARGET_SCHEMA, "Map Target Companies columns")
    ...
    if ready:
        ...
        default_company_cols = [c for c in ["Associated Company (Primary)", "Company Name"] if c in contacts_df.columns]
        selectable_contact_company_cols = [c for c in contacts_df.columns if "company" in c.lower()]
        company_cols = st.multiselect(
            "Contact company columns to match on",
            options=selectable_contact_company_cols,
            default=default_company_cols or selectable_contact_company_cols[:2],
            help="Include Associated Company (Primary) and Company Name if present"
        )
        id_candidates = [c for c in ["Record ID", "Associated Email IDs", "Associated Email", "Email"] if c in contacts_df.columns]
        ...
        if go:
            ...
            st.subheader("Summary – counts per target company")
            st.dataframe(summary_df, use_container_width=True)

            st.download_button(
                "Download summary CSV",
                data=summary_df.to_csv(index=False).encode("utf-8"),
                file_name="summary_target_company_counts.csv",
                mime="text/csv"
            )
            ...
            if matched_contacts_df.empty:
                ...
                st.download_button(
                    ...
                    file_name="matched_contacts.csv",
                    mime="text/csv"
                )
            ...
            st.download_button(
                ...
                file_name="targets_no_match.csv",
                mime="text/csv"
            )
            ...
            st.download_button(
                ...
                data=xlsx_bytes,
                file_name="company_matching_results.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            )

# -----------------------------
# Tab 2: Alternate contacts schema
# -----------------------------
with tabs[1]:
    st.header("Alternate contacts schema")
    c1b, c2b = st.columns(2)
    with c1b:
        contacts_file_b = st.file_uploader("Upload Alternate Contacts CSV", type=["csv"], key="contacts2")
        contacts_df_raw_b = read_csv(contacts_file_b)
        if not contacts_df_raw_b.empty:
            st.success(f"Loaded contacts: {contacts_df_raw_b.shape[0]} rows, {contacts_df_raw_b.shape[1]} columns")

    with c2b:
        targets_file_b = st.file_uploader("Upload Target Companies CSV", type=["csv"], key="targets2")
        targets_df_raw_b = read_csv(targets_file_b)
        if not targets_df_raw_b.empty:
            st.success(f"Loaded targets: {targets_df_raw_b.shape[0]} rows, {targets_df_raw_b.shape[1]} columns")

    if not contacts_df_raw_b.empty:
        mapping_contacts_b = column_mapper_ui(contacts_df_raw_b, ALT_CONTACT_SCHEMA, "Map Alternate Contacts columns")
        contacts_df_b = remap_df(contacts_df_raw_b, mapping_contacts_b)
    else:
        contacts_df_b = pd.DataFrame()

    if not targets_df_raw_b.empty:
        mapping_targets_b = column_mapper_ui(targets_df_raw_b, TARGET_SCHEMA, "Map Target Companies columns")
        targets_df_b = remap_df(targets_df_raw_b, mapping_targets_b)
    else:
        targets_df_b = pd.DataFrame()

    st.markdown("---")

    ready_b = not contacts_df_b.empty and not targets_df_b.empty and "Company" in targets_df_b.columns
    if ready_b:
        st.subheader("Run matching (Alternate)")
        suggested_cols = [c for c in ["companyName", "Company", "Company Name"] if c in contacts_df_b.columns]
        selectable_company_cols_b = [c for c in contacts_df_b.columns if "company" in c.lower()]
        company_cols_b = st.multiselect(
            "Contact company columns to match on",
            options=selectable_company_cols_b,
            default=suggested_cols or selectable_company_cols_b[:2]
        )
        id_candidates_b = [c for c in ["vmid", "companyId", "Email", "linkedInProfileUrl", "Person Linkedin Url"] if c in contacts_df_b.columns]

        go_b = st.button("Match now (Alternate)", type="primary")
        if go_b:
            with st.spinner("Matching..."):
                summary_b, matched_b, no_match_b = run_matching(
                    contacts_df=contacts_df_b,
                    targets_df=targets_df_b,
                    target_company_col="Company",
                    contact_company_cols=company_cols_b,
                    id_col_candidates=id_candidates_b,
                    scorer_name=scorer_name,
                    score_cutoff=score_cutoff,
                )
