...
def best_state_match(location: str) -> str:
    ...
    if not candidates:
        return ""
    state, token, score = max(candidates, key=lambda x: x[2])
    return state if score >= STATE_FUZZ_THRESHOLD else ""
...
def segment(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    if "companyName" in df.columns:
        df["_CompanyType"] = df["companyName"].apply(classify_company_type)
    else:
        df["_CompanyType"] = "Bank"

    if "title" in df.columns:
        df["_Focus"] = df["title"].apply(classify_focus)
    else:
        df["_Focus"] = "All Others"

    if "location" in df.columns:
        df["_State"] = df["location"].apply(best_state_match)
    else:
        df["_State"] = ""
...
def message_pack(state: str, company_type: str, focus: str) -> Dict[str, str]:
    ...
    conn = (
        f"Hi {{FirstName}},\n\n"
        f"I’m connecting with {dom['BANKING']} leaders across {state} who are driving projects in AI, data and automation. "
        f"It would be great to connect and share insights ahead of a summit in Charlotte this November.\n\n"
        f"Best regards, Mark"
    )
    f1 = (
        f"Hi {{FirstName}},\n\n"
        f"I’m reaching out to senior {dom['BANKING']} leaders in {state} as we prepare for the Banking Transformation Summit in Charlotte this Nov "
    ...
    return {"Connection": conn, "FollowUp1": f1, "FollowUp2": f2}

def build_summary_df(buckets: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    rows = []
    for seg_name, df in buckets.items():
        # Parse segment: "<State> - <CompanyType> - <Focus>"
        try:
            state, company_type, focus = [x.strip() for x in seg_name.split(" - ", 2)]
        except Exception:
            state, company_type, focus = "", "Bank", "All Others"
        msgs = message_pack(state, company_type, focus)
        rows.append({
            "Segment": seg_name,
            "Count": len(df),
            "Connection": msgs["Connection"],
            "Follow Up 1": msgs["FollowUp1"],
            "Follow Up 2": msgs["FollowUp2"],
        })
...
st.caption("Unmapped fields are left blank. No rows are dropped.")
...
if st.button("Build segments and download ZIP"):
    ...
    # Create per-segment CSVs in a ZIP with the summary workbook
    zip_buf = io.BytesIO()
    with zipfile.ZipFile(zip_buf, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
        ...
        for name, d in buckets.items():
            # Ensure standard column order first
            ...
            # Safe filename: "<State> - <Type> - <Focus>.csv"
            fname = _sanitize_filename(f"{name}.csv")
    ...
    st.info("Edit keyword lists in FOCUS_BUCKETS and hints in CREDIT_UNION_HINTS/BANK_HINTS. Adjust STATE_FUZZ_THRESHOLD for stricter or looser state matching.")
